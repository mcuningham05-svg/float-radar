const rivers = [
  {
    ...
    idealGaugeMin: 3.0,
    idealGaugeMax: 5.5,
    notes: "Starter planning range only. We’ll tighten these ranges river by river."
  },
  {
    name: "Mulberry River",
    ...
    idealGaugeMin: 2.0,
    idealGaugeMax: 4.5,
    notes: "Mulberry rises and falls fast after rain."
  },
  {
    name: "White River",
    ...
    idealGaugeMin: 2.5,
    idealGaugeMax: 6.0,
    notes: "White River needs section-by-section logic later."
  },
  {
    name: "Elk River",
    section: "Near Noel",
    ...
    idealGaugeMin: 2.5,
    idealGaugeMax: 5.0,
    notes: "Good starter gauge, but exact float quality depends on section."
  }
];
...
function getStatus(height, min, max) {
  ...
  if (height < min) {
    return { label: "Too Low", className: "status-bad" };
  }


  if (height > max) {
    return { label: "High", className: "status-warning" };
  }
...
async function loadRivers() {
  ...
  if (!hasRealData) {
    showMessage(
      "The page loaded, but live data did not. If you opened this as a local file, your browser may be blocking the API call. Put these files on GitHub Pages or run them from a small local server."
    );
  }
}
